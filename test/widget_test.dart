...
void main() {
  testWidgets('Welcome screen renders title, phone field, and entry actions '
    ...
    // "تسجيل الدخول" appears twice by design: once as the section eyebrow
    // label above the phone field, once as the submit button's own label.
  ...
  testWidgets('Full flow: Welcome (login) -> OTP -> Registration -> Home', (
    tester,
  ) async {
    FlavorConfig.initialize(Flavor.ipsUat);
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      appRouter.go(
        ...
        extra: const RegistrationRouteArgs(
          initialLocalPhoneNumber: '7711234567',
        ),
      );
      await tester.pumpWidget(const IpsApp());
